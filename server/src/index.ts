...
// TEMPORARY DIAGNOSTIC - remove after fixing login
  app.get('/api/admin/diag-login', async (req, res) => {
    try {
      const { getConnection } = await import('./config/database');
      const conn = await getConnection();
      const [rows] = await conn.execute(
        "SELECT id, email, LEFT(password, 20) as hash_prefix, LENGTH(password) as hash_len, is_active, tenant_id FROM users WHERE email = 'alfons@tecnocim.com' AND tenant_id = (SELECT id FROM tenants WHERE slug = 'tecnocim')"
      ) as any;
      const bcrypt = require('bcryptjs');
      const freshHash = await bcrypt.hash('Tecnocim2026!', 10);
      const [updateResult] = await conn.execute(
        "UPDATE users SET password = ? WHERE email = ? AND tenant_id = (SELECT id FROM tenants WHERE slug = 'tecnocim')",
        [freshHash, 'alfons@tecnocim.com']
      ) as any;
      conn.release();
      res.json({ success: true, data: { users: rows, updateAffected: updateResult.affectedRows, newHashLen: freshHash.length } });
